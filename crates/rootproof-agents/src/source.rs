use rootproof_core::{EvidenceBundle, SourceAnalysis, SourceFinding};
...
pub struct SourceAnalysisOutput {
    pub findings: Vec<SourceFindingOutput>,
}
...
pub async fn analyze_source(
...
) -> Result<SourceAnalysis, Box<dyn std::error::Error>> {
    let mut input = String::new();

    for item in &evidence.evidence {
        input.push_str(&format!("Evidence {}:\n{}\n\n", item.id, item.content));
    }
    ...
    let output: SourceAnalysisOutput = provider.extract(&input, preamble).await?;

    let findings = output
        .findings
        .into_iter()
        .map(|finding| SourceFinding {
