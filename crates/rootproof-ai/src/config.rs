...
pub fn save_config(config: &RootProofConfig) -> Result<PathBuf, AiError> {
    ...
    fs::create_dir_all(parent).map_err(|source| AiError::ConfigIo {
        path: parent.to_path_buf(),
        source,
    })?;

    let contents = toml::to_string_pretty(config)
        .map_err(|error| AiError::ConfigSerialize(error.to_string()))?;

    fs::write(&path, contents).map_err(|source| AiError::ConfigIo {
        path: path.clone(),
        source,
    })?;
...
pub fn load_config() -> Result<RootProofConfig, AiError> {
    ...
    let content = fs::read_to_string(&path).map_err(|source| AiError::ConfigIo {
        path: path.clone(),
        source,
    })?;

    toml::from_str(&content).map_err(|error| AiError::ConfigParse(error.to_string()))
}
