...
pub fn resolve_failure_file(repo: &Path, failure_file: &Path) -> Option<PathBuf> {
    ...
    if let Some(repo_name) = repo.file_name() {
        let components: Vec<_> = failure_file.components().collect();

        for (index, component) in components.iter().enumerate() {
            if component.as_os_str() == repo_name {
                let mut relative = PathBuf::new();

                for remaining in components.iter().skip(index + 1) {
                    relative.push(remaining.as_os_str());
                }

                if !relative.as_os_str().is_empty() && repo.join(&relative).is_file() {
...
mod tests {
    ...
    fn reads_lines_around_target() {
        ...
        let context =
            read_source_context(temp.path(), Path::new("src.rs"), 3, 1).expect("read context");

        assert!(context.content.contains("fn two()"));
    ...
    fn resolves_repository_relative_path() {
        ...
        fs::write(temp.path().join("src/lib.rs"), "fn example() {}").expect("write source file");

        let resolved = resolve_failure_file(temp.path(), Path::new("src/lib.rs"));

        assert_eq!(resolved, Some(PathBuf::from("src/lib.rs")));
    }
    ...
    fn resolves_failure_path_with_repo_prefix() {
        ...
        let repo = temp.path().join("fixtures").join("rust-failing");

        fs::create_dir_all(repo.join("src")).expect("create src directory");

        fs::write(repo.join("src/lib.rs"), "fn example() {}").expect("write source file");

        let failure_file = Path::new("fixtures/rust-failing/src/lib.rs");

        let resolved = resolve_failure_file(&repo, failure_file);
    ...
    fn returns_none_for_missing_source_file() {
        ...
        let resolved = resolve_failure_file(temp.path(), Path::new("src/missing.rs"));
